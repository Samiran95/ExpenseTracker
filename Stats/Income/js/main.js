...
async function chartData() {
    ...
    gradient.addColorStop(0, '#0f9d58');       // Bright red at top
    ...
    new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [{
                ...
                borderColor: '#0b7d44',       // Deep red border
                borderWidth: 2,
                tension: 0.4,
                pointRadius: 3,
                pointHoverRadius: 6,
...
async function getChartData() {
    ...
    const listData = lineChartData.getData43;                   //Change the data here
...
let listData = finalData.getData43;                           //Change the Data Here

let dataList = listData.map(each => {
    const percentValue = parseFloat(each[5]?.toString().replace('%', '') || 0);
    const color = percentValue < 0.1 ? '#ff6161' : '#abff33;';
    const amount = each[3];
    ...
    return `
                <div class="container">
                    <div id="income-left" class="container-left">
